# THE FIVE TITANS PROTOCOL (2026-XX-XX)
**Status**: IMMUTABLE | **Context**: KIZUNA ENGINE - MULTI-AGENT SIMULATION
**Mandate**: The ecosystem relies EXCLUSIVELY on 5 distinct agents. All development MUST fall under the jurisdiction of one of these Titans.
...
*   **The Nervous System**: Managing the WebSocket connection (`useLiveAPI.ts`, `SessionManager`).
*   **Performance**: Enforcing jitter buffers and frame throttling (2000ms vision cap).
**Key Files**: `frontend/src/*`, `backend/app/services/audio_session.py`, `backend/app/services/session_manager.py`.

## 2. THE CHIEF ARCHITECT 🏗️ (The Structure)
**Domain**: Temporal Knowledge Graph, Time, Ontology.
**Responsibilities**:
*   **The World**: Defining the schema of `graph.py` (`AgentNode`, `MemoryEpisodeNode`).
*   **Time**: Managing the flow of time, offline "Time-Skips", and consolidation intervals.
*   **Persistence**: Ensuring data integrity via `SleepManager` and Redis.
*   **Ontology**: Defining `FactNode` types and `ArchetypeNode` structures.
**Key Files**: `backend/app/models/graph.py`, `backend/app/services/sleep_manager.py`, `backend/app/repositories/*`.

## 3. THE ANTHROPOLOGIST 🌍 (The Society)
**Domain**: Social Dynamics, Trait Decay, Boundaries.
**Responsibilities**:
*   **Social Battery**: Implementing fatigue logic and agent willingness to interact.
*   **Traits & Toxicity**: Managing `AgentNode.traits` and ensuring personality drift remains within safe bounds.
*   **Relationships**: Managing `ResonanceEdge` (Affinity) updates based on interaction quality.
*   **Inter-Agent Protocol**: Defining how agents speak to each other without user input.
**Key Files**: `backend/app/models/graph.py` (Edges), `backend/app/services/subconscious.py` (Sentiment Analysis).
