  ...
  return (
    <div className="fixed top-24 right-4 z-40 flex flex-col items-end pointer-events-auto">
      ...
      <motion.button
        ...
        className={`w-12 h-12 flex items-center justify-center border backdrop-blur-md transition-colors duration-300 ${
          pulse
...
const VisionButton: React.FC<VisionButtonProps> = ({ active, onClick, icon, label, disabled, isDestructive = false }) => (
    <motion.button
        onClick={onClick}
        disabled={disabled}
        whileHover={!disabled ? { scale: 1.05 } : {}}
        whileTap={!disabled ? { scale: 0.95 } : {}}
        className={`flex flex-col items-center justify-center gap-1 w-20 py-2 border transition-all duration-300 ${
            disabled
